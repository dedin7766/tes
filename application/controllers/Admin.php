...
class Admin extends CI_Controller
{
  ...
  function index(){
    $data['calon_ketua'] = $this->db->query("select * from calon_ketua order by id_calon desc limit 10")->result();
    $data['anggota'] = $this->db->query("select * from anggota order by id_anggota desc limit 10")->result();
    $data['hasil_vote'] = $this->db->query("select * from hasil_vote order by id_vote desc limit 10")->result();
  ...
  function ganti_password_act(){
    ...
    $data['agt'] = $this->db->query("SELECT * FROM anggota  WHERE id_anggota and id_anggota='$id' ORDER BY  id_anggota='$id' ASC")->result();
      $data['hasil'] = $this->db->query("SELECT * FROM hasil_vote T, anggota A  WHERE T.id_anggota=A.id_anggota")->result();
        $data['calon_ketua'] = $this->M_perpus->get_data('calon_ketua')->result();
        $this->load->view('admin/header');
     ...
     function calon(){
      ...
      $this->M_perpus->delete_data($where, 'anggota');
        $this->M_perpus->delete_data($where, 'hasil_vote');
    ...
    function tambah_anggota_act(){
      ...
      function tambah_peminjaman_act(){
        ...
        function cetak_laporan_calon(){
          $data['calon_ketua'] = $this->M_perpus->get_data('calon_ketua')->result();
          $this->load->view('admin/header');
          $this->load->view('admin/laporan_calon',$data);
          $this->load->view('admin/footer');
        }

        function laporan_print_calon(){
           $data['calon_ketua'] = $this->M_perpus->get_data('calon_ketua')->result();
          $this->load->view('admin/laporan_print_calon',$data);
        }

        function laporan_pdf_calon(){
          $this->load->library('dompdf_gen');
          $data['calon_ketua'] = $this->M_perpus->get_data('calon_ketua')->result();
          $this->load->view('admin/laporan_pdf_calon',$data);
          ...
          $this->dompdf->render();
          $this->dompdf->stream("laporan_data_calon.pdf", array('Attachment'=>0));
          //nama file pdf yg di hasilkan
        }

         function cetak_laporan_vote(){
            $data['count'] = $this->M_perpus->get_count();
        $data['count2'] = $this->M_perpus->get_count2();
        $data['count3'] = $this->M_perpus->get_count3();
         $data['anggota'] = $this->M_perpus->getByDesc();
          $data['calon_ketua'] = $this->M_perpus->get_data('calon_ketua')->result();
          $this->load->view('admin/header');
          $this->load->view('admin/laporan_vote',$data);
          $this->load->view('admin/footer');
        }

        function laporan_print_vote(){
            $data['count'] = $this->M_perpus->get_count();
        $data['count2'] = $this->M_perpus->get_count2();
        $data['count3'] = $this->M_perpus->get_count3();
         $data['anggota'] = $this->M_perpus->getByDesc();
           $data['calon_ketua'] = $this->M_perpus->get_data('calon_ketua')->result();
          $this->load->view('admin/laporan_print_vote',$data);
        }

        function laporan_pdf_vote(){
            $data['count'] = $this->M_perpus->get_count();
        $data['count2'] = $this->M_perpus->get_count2();
        $data['count3'] = $this->M_perpus->get_count3();
         $data['anggota'] = $this->M_perpus->getByDesc();
          $this->load->library('dompdf_gen');
          $data['calon_ketua'] = $this->M_perpus->get_data('calon_ketua')->result();
          $this->load->view('admin/laporan_pdf_vote',$data);
